...
setwd("/Users/ivor.williams/Documents/CRED/Fish Team/ASRAMP15")

###EXAMPLE CODE FOR MERGING WITH ACCESS DATABASE OUTPUT ....
x<-read.csv("REA_FISH_BASE_FOR_R.csv")    #NB... 'REA FISH BASE FOR R' IS THE ACCESS QUERY THAT GENERATES OUTPUT TO MATCH THE DATA_COLS ABOVE


x$DATE_<-as.POSIXct(strptime(x$DATE_, "%m/%d/%y"))
x$DATE_<-as.character(x$DATE_)
#select only third leg
###x<-x[x$DATE_ >"2014-04-15",]
x<-droplevels(x)
...
"REP",  "REPLICATEID", "DIVER", "HABITAT_TYPE", "DEPTH", "DATE_",
...
tmp.lev<-levels(wd$HABITAT_TYPE); head(tmp.lev)
levels(wd$HABITAT_TYPE)<-c(tmp.lev, "UNKNOWN")
...
wd[is.na(wd$HABITAT_TYPE),"HABITAT_TYPE"]<-"UNKNOWN"
...
#family.cols<-family.cols[-70] ## drop level UNKNOWN

## ADDED THIS:
family.cols[-1]->family.cols
...
divers<-c("Andrew", "Jake", "JMM-James?", "Kelvin", "Louise", "Paula")
...
SPECIES_OF_INTEREST<-c("ACNC", "BAUN", "CAAB", "CAME", "CEAR", "CEMI", "CEUR", "CHMA", "CTMA", "MYBE", "NALI", "PAIN", "PSBA", "PSOL", "SATI", "SCFR", "SCRU", "SCTR", "STAU", "THQU")
